...
ReturnCodes link(const uint8_t *key, size_t length, uint64_t value, uint64_t id_transaction)
{
    ...
    while (max_iterations--)
    {
        ...
        if (slot_state == SLOT_EQUALS) return ALREADY_EXISTS;
        ...
        if (slot_state == SLOT_AVAILABLE || slot_state == SLOT_DELETED) {
            ...
            HashIdentityData buffer = identity_hashed_map[index].staged_data;
            
            add_action_to_transaction(id_transaction, PROVIDER_TRANSACTION_ID, sizeof(HashIdentityData), &buffer);
